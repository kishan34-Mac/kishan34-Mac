...
def fetch_bento_metrics(username="octocat"):
    total_year = "0"
    contrib_url = f"https://github.com/users/{username}/contributions"
    html_text = ""
    try:
        import requests
        resp = requests.get(contrib_url, headers={"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)"}, timeout=10)
    ...
    try:
        import requests
        resp2 = requests.get(repos_url, headers={"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)"}, timeout=10)
    ...
    lang_percentages = []
    colors = ["#ffffff", "#8b949e", "#565e69", "#30363d", "#21262d"]
    for idx, (lang, b) in enumerate(sorted(lang_totals.items(), key=lambda x: -x[1])[:4]):
